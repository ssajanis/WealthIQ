...
import { listLoans, createLoan, deleteLoan } from '@/lib/sheets';
...
export async function GET() {
  ...
  try {
    ...
    console.error(err);
    return NextResponse.json({ ok: false, error: 'Failed to fetch loans' }, { status: 500 });
  }
}
...
export async function POST(request: Request) {
  ...
  try {
    ...
    const data = await createLoan(parsed.data);
